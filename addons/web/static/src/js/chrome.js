...
instance.web.UserMenu =  instance.web.Widget.extend({
    ...
    on_menu_help: function() {
        window.open('http://help.maxive.pe', '_blank');
    },
    ...
    on_menu_account: function() {
        ...
        if (!this.getParent().has_uncommitted_changes()) {
            ...
            P.call('get_param', ['database.uuid']).then(function(dbuuid) {
                ...
                instance.web.redirect('https://accounts.maxive.pe/oauth2/auth?'+$.param(params));
            }).fail(function(result, ev){
                ev.preventDefault();
                instance.web.redirect('https://accounts.maxive.pe/web');
            });
        }
    },
